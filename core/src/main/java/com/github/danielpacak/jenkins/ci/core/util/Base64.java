...
public final class Base64 {
    ...
    * meaning. The table is only 7 bits wide, as the 8th bit is discarded during decoding.
    */
   ...
   public static final String encodeString(final String content) {
      byte[] bytes;
      try {
   ...
   private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset) {
      ...
      // Example: DkLE
         ...
         int outBuff = ((DEC[source[srcOffset]] & 0xFF) << 18) | ((DEC[source[srcOffset + 1]] & 0xFF) << 12)
               | ((DEC[source[srcOffset + 2]] & 0xFF) << 6) | ((DEC[source[srcOffset + 3]] & 0xFF));
    ...
    * @return decoded data
    * @throws IllegalArgumentException the input is not a valid Base64 sequence.
    */
      ...
      for (int i = off; i < off + len; i++) {
         ...
         if (EQUALS_SIGN_DEC <= sbiDecode) {
            ...
            throw new IllegalArgumentException(MessageFormat.format(
                  "Bad Base64 input character at {0} : {1} (decimal)", i, source[i] & 0xff));
      }
